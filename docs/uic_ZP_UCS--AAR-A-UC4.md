---
layout: default
title: ZP_UCS--AAR-A-UC4
has_children: true
nav_order: 21
---


# ZP_UCS--AAR-A-UC4 : Automated design assessments for sustainability.
...
AAR-A-UC4 aims at providing automated assessments specific to the DGNB certification system during the exploration of design options. In this use case, architects are engaged in the design process and wish to evaluate various alternative design options, referred to as the design space. The Probono decision support tool facilitates this by allowing architects to precisely define their design space and systematically sample it through coverage analysis. Each sampled design is then assessed against selected DGNB criteria, and the results are clustered based on their similarities in terms of DGNB outcomes. This process not only visualizes the clusters for better insight but also enables architects to focus on specific clusters to discover additional designs that meet their chosen criteria. Ultimately, this use case supports the derivation of high-performing configurations for building management and energy management systems to achieve desired outcomes related to DGNB indicators.
