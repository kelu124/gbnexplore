...
# ZP_UCS--PRG-A-UC4 : Performance monitoring for Green Building.
...
PRG-A-UC4 aims at detecting performance issues within the Green Building Neighbourhood (GBN) and alerting the technical team responsible for its exploitation. The user will utilize the Digital Twin (DT) to receive notifications about any equipment or areas exhibiting unexpected behaviors, such as underperformance or malfunction. The DT achieves this by identifying discrepancies between simulation results and actual data collected from physical twin sensors. When such anomalies are detected, the DT will alert the team, indicating the specific areas or equipment that require attention.
